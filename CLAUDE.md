...
- **Keyboard**: `↑`/`↓`/`PgUp`/`PgDn` to scroll, `c` to clear the log view. The first `Ctrl+C` closes the TUI and keeps the pipe alive, switching to streaming raw (coloured) log lines to the current terminal; a second `Ctrl+C` (or EOF) fully exits.
